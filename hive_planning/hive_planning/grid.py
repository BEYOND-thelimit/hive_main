...
import numpy as np
from hive_planning.utils import get_movements_4n, get_movements_8n, heuristic, Vertices, Vertex
...
class OccupancyGridMap:
    ...
    def is_unoccupied(self, pos: (int, int)) -> bool: # type: ignore
        """
    ...
    def in_bounds(self, cell: (int, int)) -> bool: # type: ignore
        """
    ...
    def succ(self, vertex: (int, int), avoid_obstacles: bool = False) -> list: # type: ignore
        """
    ...
    def set_obstacle(self, pos: (int, int)): # type: ignore
        """
    ...
    def remove_obstacle(self, pos: (int, int)): # type: ignore
        """
    ...
    def local_observation(self, g_pos_1: (int, int), view_range: int = 2) -> Dict: # type: ignore
        """
...
"""
    upgrade version local_observation function below
    if you want use it. chack exampl :)
    ex) robot_positon_list=[(1,1), (2,2), (3,3)] 
       local_observation(robot_position_list, view =2 )  <- this line in the main.py


    def local_observation(self, g_positions: List[(int, int)], view_range: int = 2) -> Dict:
        """"""
        :param g_positions: list of positions of robots in the global map frame
        :param view_range: how far ahead we should look
        :return: dictionary of new observations
        """"""
        nodes = []
        for g_pos in g_positions:
            (px, py) = g_pos
            nodes_temp = [(x, y) for x in range(px - view_range, px + view_range + 1)
                        for y in range(py - view_range, py + view_range + 1)
                        if self.in_bounds((x, y))]
            nodes += nodes_temp
        return {node: UNOCCUPIED if self.is_unoccupied(pos=node) else OBSTACLE for node in nodes}
...
class SLAM:
    ...
    def c(self, u: (int, int), v: (int, int)) -> float: # type: ignore
        """
    ...
    def rescan(self, g_pos_1: (int, int)): # type: ignore

        # rescan local area
        local_observation = self.ground_truth_map.local_observation(g_pos_1=g_pos_1, view_range=self.view_range)
...
# """
#     # same algorithm local_observation

#     def rescan(self, g_positions: List[(int, int)]):

#         # rescan local area
#         local_observation = self.ground_truth_map.local_observation(g_positions=g_positions, view_range=self.view_range)
