...
from hive_planning.priority_queue import PriorityQueue, Priority
from hive_planning.grid import OccupancyGridMap
import numpy as np
from hive_planning.utils import heuristic, Vertices
...
class DStarLite:
    def __init__(self, map: OccupancyGridMap, s_start: (int, int), s_goal: (int, int)):
        ...
        self.U.insert(self.goal, Priority(heuristic(self.s_start, self.goal), 0))
    
    def set_goal(self,s_start : (int, int), new_goal:(int,int)):
        self.new_edges_and_old_costs = None
        self.goal = new_goal
        self.s_start=s_start
        return self.goal
